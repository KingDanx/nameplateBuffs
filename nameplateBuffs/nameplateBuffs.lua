...
local C_NamePlate_GetNamePlateForUnit, C_NamePlate_GetNamePlates, CreateFrame, UnitBuff, UnitDebuff, UnitName,
    UnitIsUnit, UnitIsPlayer, UnitPlayerControlled, UnitIsEnemy, UnitIsFriend, GetSpellInfo, table_sort, strmatch,
    format, wipe, pairs, GetTime, math_floor = C_NamePlate.GetNamePlateForUnit, C_NamePlate.GetNamePlates, CreateFrame,
    C_UnitAuras.GetBuffDataByIndex, C_UnitAuras.GetDebuffDataByIndex, UnitName, UnitIsUnit, UnitIsPlayer,
    UnitPlayerControlled, UnitIsEnemy, UnitIsFriend, C_Spell.GetSpellInfo, table.sort, strmatch, format, wipe, pairs,
    GetTime, math.floor

local defaultLargeSpells, defaultMediumSpells, defaultHiddenSpells = fPB.defaultLargeSpells, fPB.defaultMediumSpells,
    fPB.defaultHiddenSpells
...
local function AddBuff(frame, type, icon, stack, debufftype, duration, expiration, my, id, scale, durationSize,
    stackSize)
...
local function FilterBuffs(isAlly, frame, type, name, icon, stack, debufftype, duration, expiration, caster, spellID, id)
    ...
    if not listedSpell then
        ...
        if (type == "HARMFUL" and (db.showDebuffs == 1 or ((db.showDebuffs == 2 or db.showDebuffs == 4) and my))) or
            (type == "HELPFUL" and (db.showBuffs == 1 or ((db.showBuffs == 2 or db.showBuffs == 4) and my))) then
        ...
        if (type == "HARMFUL" and (db.showDebuffs == 4 and not my)) or
            (type == "HELPFUL" and (db.showBuffs == 4 and not my)) then
            return
        end
        if (listedSpell.show == 1) or (listedSpell.show == 2 and my) or (listedSpell.show == 4 and isAlly) or
            (listedSpell.show == 5 and not isAlly) then
            AddBuff(frame, type, icon, stack, debufftype, duration, expiration, my, id, listedSpell.scale,
                listedSpell.durationSize, listedSpell.stackSize)
            return
        end
    end
end
...
local function UpdateBuffIconOptions(self)
    ...
    if db.borderStyle == 1 then
        self.border:SetTexture("Interface\\Addons\\nameplateBuffs\\texture\\border.tga")
...
function fPB.AddNewSpell(spell)
    ...
    if db.Spells[spell] and not defaultSpell then
        if spellID then
            DEFAULT_CHAT_FRAME:AddMessage(chatColor .. L["Spell with this ID is already in the list. Its name is "] ..
                                              linkColor .. "|Hspell:" .. spellID .. "|h[" .. GetSpellInfo(spellID) ..
                                              "]|h|r")
            return
        else
    ...
    local spell = GetSpellInfo(spellID)
    if spellID and spell.name then
...
function fPB.ChangeSpellID(oldID, newID)
    if db.Spells[newID] then
        DEFAULT_CHAT_FRAME:AddMessage(chatColor .. L["Spell with this ID is already in the list. Its name is "] ..
                                          linkColor .. "|Hspell:" .. newID .. "|h[" .. GetSpellInfo(newID) .. "]|h|r")
        return
    end
    db.Spells[newID] = {}
    ...
    DEFAULT_CHAT_FRAME:AddMessage(GetSpellInfo(newID) .. chatColor .. L[" ID changed "] .. "|r" ..
                                      (tonumber(oldID) or "nil") .. chatColor .. " -> |r" .. newID)
    UpdateAllNameplates(true)
    fPB.BuildSpellList()
end
...
local function ConvertDBto2()
    local temp
    for _, p in pairs(nameplateBuffsDB.profiles) do
        if p.Spells then
            temp = {}
...
local function Initialize()
    if nameplateBuffsDB and (not nameplateBuffsDB.version or nameplateBuffsDB.version < 2) then
        ConvertDBto2()
    end
    ...
    function SlashCmdList.NAMEPLATEBUFFS(msg, editBox)
        Settings.OpenToCategory(fPBMainOptions)
        Settings.OpenToCategory(fPBSpellsList)
        Settings.OpenToCategory(fPBMainOptions)
    end
end
